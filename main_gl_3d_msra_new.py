    ...
    'diffusion':
        {
            ...
            'channels': 32,#64
            'nheads': 8,#8
            'diffusion_embedding_dim': 64,#128
            'beta_start': 0.0001,
            'beta_end': 0.5,
            'num_steps': 50,
            'schedule': "cosine"
...
def save_state(model, optimizer, scheduler, epoch_no, foldername):
    model_name = 'msra_3d_ours_' + str(args.output_n) + 'frames_ckpt'
...
#                             renorm_all_joints_j[:, :] = renorm_pose_j
#                             # renorm_all_joints_j[:, index_to_ignore] = renorm_all_joints_j[:, index_to_equal]
#                             error = mpjpe_error(torch.from_numpy(renorm_all_joints_j).view(output_n, 21, 3),
#                                                 torch.from_numpy(renorm_c_target_i).view(output_n, 21, 3))
...
#
#                 renorm_all_joints_seq[:, :, :] = renorm_pose
#                 renorm_all_joints_seq[:, :, :] = renorm_all_joints_seq[:, :, :]
#
#                 mpjpe_p3d_h36 = torch.sum(torch.mean(
#                     torch.norm(renorm_c_target.view(-1, output_n, 21, 3)
#                                - renorm_all_joints_seq.view(-1, output_n, 21, 3), dim=3),
#                     dim=2), dim=0)
...
#
#                 mpjpe_current = mpjpe_error(renorm_all_joints_seq.view(-1, output_n, 21, 3),
#                                             renorm_c_target.view(-1, output_n, 21, 3))
#                 # print(332, np.shape(renorm_all_joints_seq.view(-1, output_n, 21, 3))) #[32, 25, 21, 3]
...
def calculate_average_accuracy(total_action, actions, output_n):
    ################# write to csv
    file_path_msra = '../sota_results_analyse/' + 'msra_' + str(output_n) + '.csv'
    # actions.append('Average')
    # actions.append('Methods')
    file_path = file_path_msra
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        dataset = MSRA(data_dir, input_n, output_n, args.skip_rate_train, split=0, miss_rate=(args.miss_rate / 100),
        ...
        valid_dataset = MSRA(data_dir, input_n, output_n, args.skip_rate_val, split=1, miss_rate=(args.miss_rate / 100),
                             miss_type=args.miss_type, all_data=all_data, joints=args.joints)
        ...
        all_mpjpe_actions = []
        all_batch_number = []
        model_name_s = 'msra_3d_ours_' + str(5) + 'frames_ckpt'
        model_name_l = 'msra_3d_ours_' + str(args.output_n) + 'frames_ckpt'
        ...
        for i, action in enumerate(actions):
            test_dataset = MSRA(data_dir, input_n, output_n, skip_rate, split=2, miss_rate=(args.miss_rate / 100),
