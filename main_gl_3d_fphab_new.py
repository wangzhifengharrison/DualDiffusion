...
from utils.fphab_3d import FPHAB
...
def save_state(model, optimizer, scheduler, epoch_no, foldername):
    model_name = 'fphab_3d_ours_' + str(args.output_n) + 'frames_ckpt'
...
def calculate_average_accuracy(total_action, actions, output_n):
    ################# write to csv
    file_path_fphab = '../sota_results_analyse/' + 'fphab_' + str(output_n) + '.csv'
    # actions.append('Average')
    # actions.append('Methods')
    file_path = file_path_fphab
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        dataset = FPHAB(data_dir, input_n, output_n, args.skip_rate_train, split=0, miss_rate=(args.miss_rate / 100),
        ...
        valid_dataset = FPHAB(data_dir, input_n, output_n, args.skip_rate_val, split=1, miss_rate=(args.miss_rate / 100),
                             miss_type=args.miss_type, all_data=all_data, joints=args.joints)
        ...
        actions = ['open_juice_bottle','close_juice_bottle','pour_juice_bottle','open_peanut_butter','close_peanut_butter', 'sprinkle',
        ...
        all_mpjpe_actions = []
        all_batch_number = []
        model_name_s = 'fphab_3d_ours_' + str(5) + 'frames_ckpt'
        model_name_l = 'fphab_3d_ours_' + str(args.output_n) + 'frames_ckpt'
        ...
        for i, action in enumerate(actions):
            test_dataset = FPHAB(data_dir, input_n, output_n, skip_rate, split=2, miss_rate=(args.miss_rate / 100),
