...
from utils import data_utils_for_msra
...
class MSRA(Dataset):
    ...
    def __init__(self, data_dir,input_n,output_n,skip_rate, actions=None, split=0,miss_rate=0.2,
        ...
        :param sample_rate:
        """
        self.path_to_data = os.path.join(data_dir,'datasets/msra')
        self.split = split
        self.in_n = input_n
        self.out_n = output_n
        ...
        subs = np.array([[0, 2, 3, 4, 5,6,7], [8], [1]])
        ...
        for subj in subs:
            for action_idx in np.arange(len(acts)):
                ...
                if self.split <= 1:
                    ...
                    filename = '{0}/P{1}/{2}.txt'.format(self.path_to_data, subj, action)
                    ## read .csv files
                    the_sequence = data_utils_for_msra.readCSVasFloat(filename)
