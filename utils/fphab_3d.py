...
from utils import data_utils_for_fphab
...
class FPHAB(Dataset):
    ...
    def __init__(self, data_dir, input_n, output_n, skip_rate, actions=None, split=0, miss_rate=0.2,
        ...
        :param sample_rate:
        """
        self.path_to_data = os.path.join(data_dir,'datasets/fphab')
        self.split = split
        self.in_n = input_n
        self.out_n = output_n
        ...
        subs = np.array([['Subject_1','Subject_3','Subject_4','Subject_5'], ['Subject_6'], ['Subject_2']])
        # acts = data_utils.define_actions(actions)
        if actions is None:
            acts = ['open_juice_bottle','close_juice_bottle','pour_juice_bottle','open_peanut_butter','close_peanut_butter', 'sprinkle',
               'stir','open_milk','close_milk','pour_milk','drink_mug','put_tea_bag','put_salt',
                'pour_liquid_soap', 'wash_sponge',  'scratch_sponge', 'squeeze_sponge', 'use_flash', 'write', 'tear_paper',
             'squeeze_paper', 'open_letter', 'take_letter_from_enveloppe', 'read_letter', 'flip_pages', 'use_calculator', 'light_candle', 'charge_cell_phone', 'unfold_glasses',
                'clean_glasses', 'give_coin', 'receive_coin', 'give_card','pour_wine', 'toast_wine', 'handshake', 'high_five'
                ]
        else:
            acts = actions
        ...
        subs = subs[split]
        key = 0
        # read subaction from the file
        with open(self.path_to_data + '/' + 'all_subject_infos.json', 'r') as f:
            all_subject_infos = json.load(f)
        #{'Subject_1': {'charge_cell_phone': ['1', '2', '3', '4'],
        # print(all_subject_infos)
        for subj in subs:
            for action_idx in np.arange(len(acts)):
                action = acts[action_idx]
                if self.split <= 1:
                    for subact in all_subject_infos[subj][action]:  # subactions
                        print("Reading subject {0}, action {1}, subaction {2}".format(subj, action, subact))
                        filename = '{0}/{1}/{2}_{3}.txt'.format(self.path_to_data, subj, action, subact)
                        the_sequence = data_utils_for_fphab.readCSVasFloat(filename)
...
#     output_n = 25
#     skip_rate = 1
#     dataset = Datasets('../datasets/',input_n,output_n,skip_rate, split=0)
